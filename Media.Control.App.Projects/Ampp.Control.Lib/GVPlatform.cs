...
namespace Ampp.Control.lib
{
    internal class GVPlatform
    {
        ...
        public async Task<bool> StartNotificationsListenerAsync()
        {
            ...
            {
                Console.WriteLine($"Failed to start notification listener {ex}");
                return false;
            }
        }
        ...
        {
            Console.WriteLine($"SignalR hub connection closed.  Error : {arg?.Message}");
        ...
        {
            Console.WriteLine($"SignalR hub connection lost. Reconnecting.  Error : {arg?.Message}");
            return Task.CompletedTask;
        }
        ...
        private async Task<bool> GetAccessTokenAsync()
        {
            try
            {
                ...
                {
                    Console.WriteLine($"Failed to get access token {result?.ReasonPhrase}");
                }
            }
            catch (Exception ex)
            {
