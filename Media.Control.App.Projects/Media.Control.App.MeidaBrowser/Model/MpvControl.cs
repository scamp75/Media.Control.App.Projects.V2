...
namespace Mpv.Player.App
{
    ...
    public class MpvControl
    {
        ...
        public EnuStat Statue { get; set; } = EnuStat.Eject;
...
//            Pause();
        ...
        private double GetFps(string path)
        {
            ...
            try
            {
                ...
                fps = Convert.ToDouble(MediaInfo.GetInfo(MediaInfoStreamKind.Video, "FrameRate"));
            }
            catch (Exception ex)
            {

                System.Windows.MessageBox.Show($"Open Error {path} : {ex.Message}");
